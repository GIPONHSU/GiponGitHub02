...
import { updateBlackZombie } from "./zombies/BlackZombie";
...
export function updateZombies(
  ...
  this.zombies.forEach((z) => {
    ...
    if (z.introState === "walking_in") {
      ...
      const walkSpeed =
        (z.type === "zombie_small" ? 80 : 50) * (z.speedMultiplier || 1) * 2;
    ...
    if (
      ...
      if ((z as any).dyingTimer <= 0) {
        ...
        // Small zombie particles
        let mainColor = "#22c55e";
        if (z.type === "zombie_bomb") mainColor = "#ef4444";
        else if (z.type === "zombie_bouncing") mainColor = "#be185d";
        else if (z.type === "zombie_big") mainColor = "#9333ea";

        for (let i = 0; i < 35; i++) {
          const ang = Math.random() * Math.PI * 2;
          const spd = Math.random() * (z.type === "zombie_small" ? 150 : 300);
          this.particles.push({
            x: z.x,
            y: z.y,
            vx: Math.cos(ang) * spd,
            ...
            color: mainColor,
            size: Math.random() * 5 + (z.type === "zombie_small" ? 3 : 6),
          });
        }
      }
      return;
    ...
    if (z.type === "zombie_boss" && (z as any).isDying) {
      ...
      (z as any).bossDyingTimer = Math.max(
        0,
        ((z as any).bossDyingTimer ?? 3.0) - dt,
      );
      (z as any).bossDyingExplosionTimer =
        ((z as any).bossDyingExplosionTimer ?? 0) + dt;
    ...
    GameUtils.applyLaunchPadAvoidance(this, z);
    z.x += z.vx * dt;
    z.y += z.vy * dt;
