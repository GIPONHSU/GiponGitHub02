...
export function spawnZombie(engine: GameEngine) {
  const humanPlayers = engine.participants
    ? engine.participants.filter((p) => !p.isAI).length
    : 0;
  const playerCount = humanPlayers > 0 ? humanPlayers : 1;
  const scaleMultiplier = Math.min(1.8, 1.0 + (playerCount - 1) * 0.267);

  const currentSmalls = engine.zombies.filter(
    (z) => z.type === "zombie_small" && !z.markForDeletion,
  ).length;
  const currentBigs = engine.zombies.filter(
    (z) =>
      (z.type === "zombie_big" ||
        z.type === "zombie_bomb" ||
  ...
  let r = isBig ? 64 : 14.4; // 24 * 0.6 = 14.4
...
export function spawnGoldenZombie(engine: GameEngine) {
  ...
  if (
    engine.zombies.some((z) => z.type === "zombie_golden" && !z.markForDeletion)
  ) {
  ...
  for (let i = 0; i < 10; i++) {
    engine.zombies.push({
      ...
      type: "zombie_small",
      x: sx,
      y: sy, // start at same position, updated by behavior
      vx: 0,
      vy: 0,
      radius: 14.4, // Small zombie radius
      mass: 8,
      markForDeletion: false,
      hp: 2,
      maxHp: 2,
...
export function spawnBlackZombie(engine: GameEngine) {
  // Check if black zombie already exists
  if (
    engine.zombies.some((z) => z.type === "zombie_black" && !z.markForDeletion)
  ) {
    return; // Only 1 black zombie at a time
  }

  const r = 64; // Same as golden zombie
  const R = 480;
  const leftCenterX = 540;
  const rightCenterX = 1380;
  const centerY = engine.activeArenaCenterY ?? 540;
  ...
  const blackId = "z_black_" + Math.random();
  ...
  const blackZombie = {
    ...
    bossAttackState: "idle",
    bossAttackTimer: 0,
    bossNextAttackTime: 2.5,
  } as Zombie;

  engine.zombies.push(blackZombie);
}

export function spawnItemOrObstacle(
  engine: GameEngine,
  forcedType?: "item_crate" | "obstacle_barrel",
) {
  const hasSuperPlayer = engine.tops.some(
    (t) =>
      (t.superTimer !== undefined && t.superTimer > 0) ||
      (t.rainbowSuperTimer !== undefined && t.rainbowSuperTimer > 0),
  );
  const currentCrates = engine.obstacles.filter(
    (o) => (o.type as string) === "item_crate" && !o.markForDeletion,
  ).length;
  const currentBarrels = engine.obstacles.filter(
    (o) => (o.type as string) === "obstacle_barrel" && !o.markForDeletion,
  ).length;

  // Map forced item_crate to obstacle_barrel dynamically to prevent type narrowing while honoring the disable request
  const targetForcedType =
    forcedType === "item_crate"
      ? ("obstacle_barrel" as "item_crate" | "obstacle_barrel")
      : forcedType;
  ...
  engine.obstacles.push({
    // pushing to obstacles is fine for all these static drops
