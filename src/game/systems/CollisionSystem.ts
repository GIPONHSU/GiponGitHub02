...
export function handleCollision(engine: GameEngine, a: Entity, b: Entity) {
    ...
    if (a.type === 'top' && (a as any).struggleTriumphTimer > 0 && b.type.startsWith('zombie')) {
        return;
    }
    if (b.type === 'top' && (b as any).struggleTriumphTimer > 0 && a.type.startsWith('zombie')) {
        return;
    }

    // Boss / Black Zombie struggle charge clash trigger
    const isBossStruggleColliding = 
        ((a.type === 'zombie_boss' || a.type === 'zombie_black') && (a as any).bossAttackState === 'struggle_charge' && b.type === 'top' && (a as any).bossWarningTargetId === b.id) ||
        ((b.type === 'zombie_boss' || b.type === 'zombie_black') && (b as any).bossAttackState === 'struggle_charge' && a.type === 'top' && (b as any).bossWarningTargetId === a.id);
    if (isBossStruggleColliding) {
        const boss = ((a.type === 'zombie_boss' || a.type === 'zombie_black') ? a : b) as Zombie;
    ...
    if ((z.type === 'zombie_small' || z.type === 'zombie_big' || z.type === 'zombie_bomb') && !top.isExploding && !isCurrentlyDashing) {
        ...
        if (!isSuper) {
            if (top.hitCooldown === undefined || top.hitCooldown <= 0) {
                ...
                if (top.state === 'standby' && !top.isSpinning && z.type !== 'zombie_small') {
                    const dx = z.x - top.x;
                    const dy = z.y - top.y;
