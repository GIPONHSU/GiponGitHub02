...
import {
  ...
  resolveCircleTriangleCollision,
} from "../physics";
...
export function handleCollision(engine: GameEngine, a: Entity, b: Entity) {
  if (
    (a as Top).launchPadState !== undefined ||
    (b as Top).launchPadState !== undefined
  ) {
    return;
  }

  // Ignore collisions if a player top is in a triumph dash after winning a struggle clash
  if (
    a.type === "top" &&
    (a as any).struggleTriumphTimer > 0 &&
    b.type.startsWith("zombie")
  ) {
    return;
  }
  if (
    b.type === "top" &&
    (b as any).struggleTriumphTimer > 0 &&
    a.type.startsWith("zombie")
  ) {
    return;
  }

  // Boss / Black Zombie struggle charge clash trigger
  const isBossStruggleColliding =
    ((a.type === "zombie_boss" || a.type === "zombie_black") &&
      (a as any).bossAttackState === "struggle_charge" &&
      b.type === "top" &&
      (a as any).bossWarningTargetId === b.id) ||
    ((b.type === "zombie_boss" || b.type === "zombie_black") &&
      (b as any).bossAttackState === "struggle_charge" &&
      a.type === "top" &&
      (b as any).bossWarningTargetId === a.id);
  if (isBossStruggleColliding) {
    const boss = (
      a.type === "zombie_boss" || a.type === "zombie_black" ? a : b
    ) as Zombie;
    ...
    // Immediately end the top's current behavior
    targetTop.state = "standby";
    targetTop.dashTimer = 0;
    targetTop.skillActiveTimer = 0;
    targetTop.breakoutOrbitTimer = 0;
    targetTop.coopState = undefined;
    targetTop.launchPadState = undefined;
    targetTop.superTimer = 0;
    targetTop.rainbowSuperTimer = 0;
    targetTop.standbyCenterVx = 0;
    targetTop.standbyCenterVy = 0;
    ...
    (targetTop as any).struggleAnchorX =
      boss.x + Math.cos(clashAngle) * targetDist;
    (targetTop as any).struggleAnchorY =
      boss.y + Math.sin(clashAngle) * targetDist;
  ...
  if (
    ...
    (engine.siegeStatus === "approaching" || engine.siegeStatus === "clinging")
  ) {
  ...
  if (isPlayerTopVsBoss) {
    ...
    const hasCooldown =
      top.deadlockCooldownTimer !== undefined && top.deadlockCooldownTimer > 0;
  ...
  if (a.type === "top" && b.type === "top") {
    const ta = a as Top;
    const tb = b as Top;
    const gridA = Math.max(
      1,
      Math.min(
        10,
        Math.floor(((ta.spin ?? MAX_SPIN) / (ta.maxSpin ?? MAX_SPIN)) * 10),
      ),
    );
    const gridB = Math.max(
      1,
      Math.min(
        10,
        Math.floor(((tb.spin ?? MAX_SPIN) / (tb.maxSpin ?? MAX_SPIN)) * 10),
      ),
    );
    const gridDiff = Math.abs(gridA - gridB);

    const isSuperA =
      (ta.superTimer !== undefined && ta.superTimer > 0) ||
      (ta.rainbowSuperTimer !== undefined && ta.rainbowSuperTimer > 0);
    const isSuperB =
      (tb.superTimer !== undefined && tb.superTimer > 0) ||
      (tb.rainbowSuperTimer !== undefined && tb.rainbowSuperTimer > 0);
    const hasCooldownA =
      ta.deadlockCooldownTimer !== undefined && ta.deadlockCooldownTimer > 0;
    const hasCooldownB =
      tb.deadlockCooldownTimer !== undefined && tb.deadlockCooldownTimer > 0;
  ...
  if (a.type === "item_key" || b.type === "item_key") {
    const top =
      a.type === "top" ? (a as Top) : b.type === "top" ? (b as Top) : null;
    const key =
      a.type === "item_key"
        ? (a as Item)
        : b.type === "item_key"
  ...
  if (a.type === "item_crate" || b.type === "item_crate") {
    const top =
      a.type === "top" ? (a as Top) : b.type === "top" ? (b as Top) : null;
    const crate =
      a.type === "item_crate" ? a : b.type === "item_crate" ? b : null;
  ...
  if (res) {
    ...
    if (!aIsDeadTop && !bIsDeadTop) {
      ...
      if (a.type === "top" && b.type.startsWith("zombie"))
        hitZombie(engine, a as Top, b as Zombie, res.impactForce);
      if (b.type === "top" && a.type.startsWith("zombie"))
        hitZombie(engine, b as Top, a as Zombie, res.impactForce);
    }
    ...
    if (res.impactForce > 50) {
      ...
      if (a.type === "top" && b.type === "top") {
        ...
        if (engine.gameMode === "campaign") {
          ...
          EffectSystem.addChainsawSparkParticles(
            engine,
            ta.x + res.nx * ta.radius,
            ta.y + res.ny * ta.radius,
            res.nx,
            res.ny,
            25,
            1.0,
            Math.max(ta.spin || 0, tb.spin || 0),
          );
          ...
          if (targetToDamage && attacker) {
            if ((targetToDamage as any).hp <= 0 || (attacker as any).hp <= 0) {
              ...
              EffectSystem.addChainsawSparkParticles(
                engine,
                targetToDamage.x - res.nx * targetToDamage.radius,
                targetToDamage.y - res.ny * targetToDamage.radius,
                res.nx,
                res.ny,
                15,
                1.0,
                Math.max(targetToDamage.spin || 0, attacker.spin || 0),
              );
            } else if (
              targetToDamage.rainbowSuperTimer !== undefined &&
              targetToDamage.rainbowSuperTimer > 0
            ) {
              ...
              targetToDamage.hitCooldown === undefined ||
              targetToDamage.hitCooldown <= 0
            ) {
              const baseDmg = Math.min(
                10,
                Math.max(
                  1,
                  Math.floor(
                    (attacker.spin / MAX_SPIN) * 6 +
                      (res.impactForce / 200) * 4,
                  ),
                ),
              );
              const dmg = Math.round(
                baseDmg * GameUtils.getTopScale(engine, attacker),
              );
              // 不再扣減玩家HP
            ...
            if (targetToDamage.hp <= 0 && !targetToDamage.markForDeletion) {
              ...
              if (matchAttacker) {
                ...
                if (engine.gameMode === "versus") {
                  ...
                  const part = engine.participants.find(
                    (p) => p.id === attacker.id,
                  );
                  if (part) {
        ...
        if (barrel.durability <= 0) {
          ...
          if (other.type === "top") {
            ...
            const isInvulnerable =
              (topOther.rainbowSuperTimer !== undefined &&
                topOther.rainbowSuperTimer > 0) ||
              (topOther.breakoutOrbitTimer !== undefined &&
                topOther.breakoutOrbitTimer > 0) ||
              (topOther as any).hp <= 0;
            if (isInvulnerable) {
              ...
              const bounceAngle =
                ((Math.random() < 0.5 ? 15 : -15) * Math.PI) / 180;
              ...
              if (topOther.state === "dash") {
                ...
                topOther.standbyCenterX =
                  topOther.x -
                  Math.cos(topOther.standbyAngle) *
                    getStandbyRadiusForModel(
                      topOther,
                      this,
                      topOther.standbyAngle,
                    );
                topOther.standbyCenterY =
                  topOther.y -
                  Math.sin(topOther.standbyAngle) *
                    getStandbyRadiusForModel(
                      topOther,
                      this,
                      topOther.standbyAngle,
            ...
            const isBoss_Inv =
              other.type === "zombie_boss" &&
              ((other as Zombie).bossAttackState === "warning" ||
                (other as Zombie).bossAttackState === "dash");
...
export function hitZombie(
    ...
    if (!isSuper) {
      if (top.hitCooldown === undefined || top.hitCooldown <= 0) {
        ...
        if (
          ...
          top.deflectionVx =
            (top.deflectionVx || 0) + dirVx * 150 * spinMultiplier * 1.125;
          top.deflectionVy =
            (top.deflectionVy || 0) + dirVy * 150 * spinMultiplier * 1.125;
          top.bounceTimer = 0.3;
  ...
  if (z.type === "zombie_boss") {
    ...
    if (
      ...
      (boss.introZ !== undefined && boss.introZ > 0)
    ) {
      if (
        boss.bossAttackState === "warning" &&
        ...
        if (distToWeak <= top.radius + 118.125) {
          // 157.5px decreased by 25% = 118.125px hit radius
          if (Math.random() < 0.25) {
            // 1/4 chance to break
            ...
            for (let i = 0; i < sparkCount; i++) {
              const baseAng =
                (i / sparkCount) * Math.PI * 2 + (Math.random() * 0.2 - 0.1);
      ...
      if (isDash) {
        ...
        if (top.hitCooldown === undefined || top.hitCooldown <= 0) {
          const isInvulnerable =
            (top.rainbowSuperTimer !== undefined &&
          ...
          if (!isInvulnerable) {
            ...
            top.hpLossTimer = 0.5;
            top.visualHp =
              top.visualHp !== undefined
                ? Math.max(top.hp, top.visualHp)
                : top.hp;
            EffectSystem.addChainsawSparkParticles(
              engine,
              top.x,
      ...
      if (top.state === "dash") {
        ...
        const dotDash =
          (top.dashDirectionX || 0) * nx + (top.dashDirectionY || 0) * ny;
        if (dotDash < 0) {
        ...
        const bSin = Math.sin(bounceAngle);
        const rDx =
          (top.dashDirectionX ?? 0) * bCos - (top.dashDirectionY ?? 0) * bSin;
        const rDy =
          (top.dashDirectionX ?? 0) * bSin + (top.dashDirectionY ?? 0) * bCos;
  ...
  if (z.type === "zombie_bouncing") {
    const bz = z as any;
    if (
      bz.bouncingAttackState === "warning" ||
      bz.bouncingAttackState === "bouncing" ||
  ...
  const killsZombie =
    isDashing && z.hp - 1 * GameUtils.getTopScale(engine, top) <= 0;
  const isHighSpin = ((top.spin ?? 1000) / (top.maxSpin || 1000)) * 10 >= 5.0;

  const baseDeflect = isDashing
    ? killsZombie || isHighSpin
      ? 80
      : 380
    : killsZombie || isHighSpin
      ? 60
      : 180; // Minimal shock back on high spin or slice kills to maintain smooth forward momentum
  ...
  const finalDeflectPower =
    (baseDeflect + deflectScale) * lowSpinReboundBoost * 1.125;
  ...
  if (killsZombie || isHighSpin) {
    ...
    rx = driftX * 0.9 + -nx * 0.1;
    ry = driftY * 0.9 + -ny * 0.1;
  ...
  if (isDashing) {
    if (killsZombie || isHighSpin) {
      ...
      const ty = nx;
      const dotT =
        (top.dashDirectionX || 0) * tx + (top.dashDirectionY || 0) * ty;
      const slideSign = dotT >= 0 ? 1 : -1;
      const driftX = tx * slideSign;
      const driftY = ty * slideSign;

      // Form a curved path: mix original direction with tangential slide vector and tiny outward nudge
      let newDx = (top.dashDirectionX || 0) * 0.8 + driftX * 0.45 + -nx * 0.1;
      let newDy = (top.dashDirectionY || 0) * 0.8 + driftY * 0.45 + -ny * 0.1;
      ...
      const dashBounceAngle =
        ((Math.random() < 0.5 ? 15 : -15) * Math.PI) / 180;
      const bCosD = Math.cos(dashBounceAngle);
      const bSinD = Math.sin(dashBounceAngle);
      const rDx =
        (top.dashDirectionX ?? 0) * bCosD - (top.dashDirectionY ?? 0) * bSinD;
      const rDy =
        (top.dashDirectionX ?? 0) * bSinD + (top.dashDirectionY ?? 0) * bCosD;
  ...
  let offsetPower =
    (isDashing ? (killsZombie || isHighSpin ? 4 : 12) : 6) * 1.125; // Reduced by 25% (1.125 instead of 1.5)
  ...
  if (isBoss && !top.isAI) {
    ...
    engine.particles.push({
      ...
      vx: 0,
      vy: 0,
      life: 0.3, // 0.45 / 1.5 = 0.30 (50% speedup)
      maxLife: 0.3,
      color: "#f97316", // Orange
    ...
    for (let i = 0; i < sparkCount; i++) {
      ...
      engine.particles.push({
        ...
        angle: Math.random() * Math.PI * 2,
        rotationSpeed:
          (Math.random() > 0.5 ? 1 : -1) * (4 + Math.random() * 6) * 1.5, // 50% faster rotation
      });
    }
  }
  ...
  if (z.hp <= 0) {
    ...
    if (!isAlreadyDying) {
      // === IMMEDIATE REWARDS (Points, Kills, Tickets, Explosions) ===
      ...
      // 2. Drop Ticket
      engine.spawnTicket(z.x, z.y, z.type, playerIdx);

      // 3. Increment kill tracking for the player
      ...
      // 4. 陀螺打死怪物時，產生一個放射狀的黃色星芒狀爆炸特效 (Unconditional for player kills)
      if (!top.isAI) {
        EffectSystem.spawnSkillKillExplosion(
          engine,
          contactX,
          contactY,
          isBoss,
...
export function getSpikeTriangles(engine: GameEngine) {
  ...
  const west = {
    name: "West Spike",
    x1: wBaseX,
    y1: centerY - baseWidth / 2,
    x2: wBaseX,
    y2: centerY + baseWidth / 2,
    x3: wApexX,
    y3: centerY,
    height: factor * fullHeight,
  };
  ...
  if (bounced) {
    ...
    if (e.type === "top") {
      ...
      if (top.launchPadState === "dashing") {
        ...
        const dotLaunch =
          ((top as any).launchPadDirX || 0) * wx +
          ((top as any).launchPadDirY || 0) * wy;
        if (dotLaunch > 0) {
          (top as any).launchPadDirX =
            ((top as any).launchPadDirX || 0) - 2 * dotLaunch * wx;
          (top as any).launchPadDirY =
            ((top as any).launchPadDirY || 0) - 2 * dotLaunch * wy;
        }

        // Apply exact ±15 deg random rotation
        const rLx =
          ((top as any).launchPadDirX ?? 0) * bCos -
          ((top as any).launchPadDirY ?? 0) * bSin;
        const rLy =
          ((top as any).launchPadDirX ?? 0) * bSin +
          ((top as any).launchPadDirY ?? 0) * bCos;
        ...
        const dotDash =
          (top.dashDirectionX || 0) * wx + (top.dashDirectionY || 0) * wy;
        if (dotDash > 0) {
