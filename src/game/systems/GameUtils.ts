...
function addRandomMission(top: Top) {
    ...
    const allTypes = ['zombie_small', 'zombie_big', 'zombie_bomb', 'zombie_bouncing'];
    const availableTypes = allTypes.filter(t => {
        const count = activeTypes.filter(active => active === t).length;
        return count < 2;
    });

    if (availableTypes.length === 0) return;
    
    const chosenType = availableTypes[Math.floor(Math.random() * availableTypes.length)] as keyof typeof MISSION_CONFIG;
...
export function handleZombieKillForMissions(engine: GameEngine, topId: string, zombieType: string) {
    ...
    if (freshlyCompleted > 0) {
        // Trigger power up once we reach 1 completion
        top.superTimer = 30.0;
