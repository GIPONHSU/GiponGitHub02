  ...
  engine.items.forEach((item) => {
    if (item.type === "item_ticket") {
      ...
      if (item.hoverTimer !== undefined && item.hoverTimer > 0) {
        ...
        targetX =
          (targetX - CANVAS_W / 2) / engine.camera.zoom + engine.camera.x;
        targetY =
          (targetY - CANVAS_H / 2) / engine.camera.zoom + engine.camera.y;
      ...
      if (item.hoverTimer !== undefined && item.hoverTimer > 0) {
        ...
        let targetX = CANVAS_W / 2;
        if (
          item.targetPlayerId === "top_0" ||
          item.targetPlayerId === "top_2"
        ) {
          targetX = 24 + 210 / 2;
        } else if (
          item.targetPlayerId === "top_1" ||
          item.targetPlayerId === "top_3"
        ) {
