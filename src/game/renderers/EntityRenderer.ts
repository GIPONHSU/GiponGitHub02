...
export function drawEntities(ctx: CanvasRenderingContext2D, engine: GameEngine, sprites: Record<string, HTMLCanvasElement>) {
    ...
    ctx.globalAlpha = 1.0;
        ...
        engine.zombies.forEach(z => {
            ...
            if (z.introState === 'jumping' && (z as any).introZ !== undefined && (z as any).introZ > 0) {
                ...
                let maxJumpH = z.type === 'zombie_boss' ? 30 : ((z.type === 'zombie_big' || z.type === 'zombie_bomb') ? 55 : 80);
            ...
            const anyZ = z as any;
            const isSkillPrep = anyZ.bigAttackState === 'warning' || anyZ.bouncingAttackState === 'warning' || anyZ.bossAttackState === 'warning';
            ...
            if (!isSkillPrep && z.bounceTimer !== undefined && z.bounceTimer > 0) {
                ...
                const alphaRatio = z.maxBounceTimer ? Math.max(0, Math.min(1, z.bounceTimer / z.maxBounceTimer)) : 1;
    ...
    ctx.globalAlpha = 0.6;
            ...
            if ((z.type === 'zombie_big' || z.type === 'zombie_bomb' || z.type === 'zombie_bouncing' || z.type === 'zombie_golden' || z.type === 'zombie_black') && !(z as any).isDying) {
                ...
                else if (z.type === 'zombie_golden' || z.type === 'zombie_black') ticketText = '15';
        ...
        engine.tops.forEach((top) => {
            ...
            if (top.hp > 0 && !top.isExploding) {
                ...
                const spinRatio = Math.max(0.1, Math.min(1.0, (top.spin || 0) / (top.maxSpin || 1000)));
                
                // Spin ratio drives radius
                const whirlwindBaseRad = Math.max(40, top.radius * (1.1 + 0.9 * spinRatio));
            ...
            if (top.hp > 0 && !top.isExploding && top.launchPadState === undefined) {
                ...
                ctx.fillStyle = top.hp <= (top.maxHp || 100) * 0.3 ? '#ef4444' : '#ffffff';
                ...
                ctx.font = 'bold 20px "Space Grotesk", "Microsoft JhengHei", "JetBrains Mono", sans-serif';
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
                ctx.lineWidth = 4.5;
